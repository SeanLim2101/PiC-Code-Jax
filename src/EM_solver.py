...
def curlE(field,dx,dt,field_BC_left,field_BC_right,current_t,E0,k):
    #First, set ghost cells
    
    ghost_cell_L, ghost_cell_R = field_ghost_cells_E(field_BC_left,field_BC_right,field,dx,current_t,E0,k)
...
def curlB(field,dx,dt,field_BC_left,field_BC_right,current_t,B0,k):
    #First, set ghost cells
    
    ghost_cell_L, ghost_cell_R = field_ghost_cells_B(field_BC_left,field_BC_right,field,dx,current_t,B0,k)
    field = jnp.insert(field,0,ghost_cell_L,axis=0)
    field = jnp.append(field,jnp.array([ghost_cell_R]),axis=0)  
    
    #If taking i+1 - i (since B-fields defined on edges), roll by -1 first. 
    field = jnp.roll(field,-1,axis=0)
    dFz_dx = (field[1:-1,2]-field[0:-2,2])/dx
    dFy_dx = (field[1:-1,1]-field[0:-2,1])/dx
    return jnp.transpose(jnp.array([jnp.zeros(len(dFz_dx)),-dFz_dx,dFy_dx]))
...
def field_update1(E_fields,B_fields,dx,dt_2,j,field_BC_left,field_BC_right,current_t,E0=0,k=0):
    #First, update E
    curl_B = curlB(B_fields,dx,dt_2,field_BC_left,field_BC_right,current_t,E0/3e8,k)
    E_fields += dt_2*((3e8**2)*curl_B-(j/8.85e-12))
    #Then, update B
    curl_E = curlE(E_fields,dx,dt_2,field_BC_left,field_BC_right,current_t,E0,k)
